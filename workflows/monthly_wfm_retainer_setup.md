...
## When to Run
- **Trigger:** Start of each calendar month (1st business day)
...
## Step 1: Get Active Clients and Their Hours
...
**Pagination:** The board has ~219 items total. Use pagination (500 limit per page) and filter for Active status to get only relevant clients.
...
For specialist roles, look up each client by name on the corresponding specialist board:
...
DM Client Health Check (Monday 208440216)
  ...
  |-- Get: client name, AM, AM hrs, SEO hrs, GAds hrs, PM hrs, OS hrs
  |
  v
Specialist Lookup (Monday boards)
...
Staff ID Resolution (WFM list_staff)
  ...
  |
  v
WFM Job Matching (list_jobs, status=Active)
...
3. **Multiple specialists:** If a specialist column has multiple people, use the first person listed
4. **Name mismatches:** Maintain awareness that client names might differ slightly between Monday and WFM (e.g. "Summit Developments" vs "Summit Developments Pty Ltd"). Flag these for manual review rather than guessing
5. **EDM Retainer:** The EDM retainer value feeds into the AM Hours formula calculation but does not have its own separate task in WFM
6. **Month format:** Use full month name + 4-digit year (e.g. "April 2026", not "Apr 2026" or "04/2026")

---

## WFM Authentication Issue (As of March 2026)

The WorkflowMax MCP connector is currently returning an auth error: *"No organisation ID found in token."* This needs to be resolved in the Claude app MCP settings before this workflow can execute the WFM steps. The Monday.com steps work fine.
